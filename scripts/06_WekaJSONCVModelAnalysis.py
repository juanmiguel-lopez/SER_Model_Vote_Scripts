...
def frogatxoa():
    filename = '/home/juanmi/ownCloud/Ikerkuntza/Martxan/2018_ESWA_Audio/03_scripts/RESULTS_04_results_classifiers_crossvalidated/RAVDESS_ENG/classifier_instances/RAVDESS_ENG_MultilayerPerceptron.instances'
    myinstances = Instances(jobject=serialization.read(filename))
    noes = 0
    for elem in myinstances:
...
def main():
    ...
    for myfile in json_files:
        ...
        dirName = current_path + '/RESULTS_05_Combinations_outcome/'
        if not os.path.exists(dirName):
            os.mkdir(dirName)
        ...
        #######json.dump(all_ensembles_per_combination, open(dirName + filename + '_ensembles_outcome.json', 'w'))
...
def evaluate_folds_with_classifier(base_folder, filename, classifier, comb_name, data, num_folds, num_seed):
    ...
    #json.dump(predicted_data, open(new_folder+filename+'_'+comb_name+'.instances' + '.json', 'w'))
    ...
    text = text + ("\nSeed: " + str(seed))
    text = text + ("\n")
    #text = text + (evaluation.summary("=== " + str(folds) + "-fold Cross-Validation ==="))
    #text = text + ("\n")
