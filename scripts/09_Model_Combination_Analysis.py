...
def calculate_non_recognized_instances():
    current_path = os.getcwd()
    print(current_path)
    
    #results_classifiers subfolder
    mypath = current_path + "/RESULTS_05_10_fold_results/"
    
    json_files = glob.glob(mypath+"*.json")
    
    result_path = current_path + "/RESULTS_09_Calculate_Instances_Classifiers_Outcomes/"
    ...
    for myfile in json_files:
        ...
        with open(myfile, "r") as read_file:
            ...
            instances_list = [0] * len(data[1][0])
            all_classifiers_per_instance = list()
            class_per_instance = [0] * len(data[1][0])
            
            for j in range(1, len(data[1][0])):
                #j is the instance
                current_all_classifiers_per_instance = list()
                for i in range(0, len(data[1])):
                    #i is the classifier
                    if (data[1][i][j][0]==data[1][i][j][2]):
                        instances_list[j]+=1
                    current_all_classifiers_per_instance.append(data[1][i][j][0])
                    class_per_instance[j] = data[1][i][j][2]
                all_classifiers_per_instance.append(current_all_classifiers_per_instance)
                
            #print()
            json.dump(instances_list[1:], open(result_path + os.path.basename(myfile) + '_instances_list.json', 'w'))
            json.dump(all_classifiers_per_instance, open(result_path + os.path.basename(myfile) + '_all_classifiers_per_instance.json', 'w'))
            json.dump(class_per_instance[1:], open(result_path + os.path.basename(myfile) + '_class_per_instance.json', 'w'))
                        
        print('Finished file: ' + myfile)
        ...
        for clf in clf_names:
            ...
            if Path(f).is_file():
                ...
                for elem in myinstances:
                    ...
                    #if elem.get_string_value(98) == 'no':
...
def main():
    ...
    calculate_non_recognized_instances()
